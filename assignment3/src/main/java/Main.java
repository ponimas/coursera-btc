/*
 * Main test code for Cousera cryptocurrency assignment3 Based on code by Sven Mentl and Pietro
 * Brunetti
 * 
 * Copyright: - Sven Mentl - Pietro Brunetti - Bruce Arden - Tero Keski-Valkama
 */
...
public class Main {
        ...
        public static void main(String[] args) throws NoSuchProviderException,
                ...
                Block block6 = new Block(block5.getHash(), pk_alice.getPublic());
                block6.finalize();

                System.out.println("Block6 Added ok: " + bh.processBlock(block6));

                test23();

        }

        public static int test23() throws NoSuchProviderException, NoSuchAlgorithmException,
                        InvalidKeyException, SignatureException { //The method was called test23 back then... this is line 840 on the original file
                KeyPair pk_alice = KeyPairGenerator.getInstance("RSA").generateKeyPair();
                System.out.println(
                                "Process a transaction, create a block, process a transaction, create a block, ...");

                Block genesisBlock = new Block(null, pk_alice.getPublic());
                ...
                for (int i = 0; i < 29; i++) {
                        ...
                        spendCoinbaseTx.addOutput(Block.COINBASE, pk_alice.getPublic());

                        spendCoinbaseTx.signTx(pk_alice.getPrivate(), 0);

                        spendCoinbaseTx.finalize();
                        blockHandler.processTx(spendCoinbaseTx);
                        
                        Block createdBlock = blockHandler.createBlock(pk_alice.getPublic());

                        passes = passes && createdBlock != null
                                        && createdBlock.getPrevBlockHash()
                                                        .equals(prevBlock.getHash())
